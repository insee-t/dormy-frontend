...
import { useClerk, useUser, SignedIn, useAuth } from "@clerk/nextjs";
...
const TenantsPage: React.FC = () => {
    ...
    const { client } = useClerk(); // Clerk client for API calls
        ...
        const fetchApartments = async () => {
            try {
                const [apartmentsRes, tenantsRes, defaultRes] = await Promise.all([
                    fetch(`${API_BASE_URL}/apartments/my-apartments`, { credentials: 'include' }),
                    fetch(`${API_BASE_URL}/tenants/get-tenants`, { credentials: 'include' }),
                    fetch(`${API_BASE_URL}/apartments/default-apartment`, { credentials: 'include' }),
                ]);
                    ...
                    <table className="min-w-full border-collapse bg-white shadow-md rounded-lg">
                        <thead>
                            <tr className="bg-gray-100">
                                ...
                                <th className="p-3 text-left text-sm font-semibold text-gray-700">Room Number</th>
                        ...
                        <tbody>
                            {tenants.map((tenant) => (
                                <tr key={tenant.id} className="border-t hover:bg-gray-50">
                                    <td className="p-3 text-gray-800">{tenant.firstName} {tenant.lastName}</td>

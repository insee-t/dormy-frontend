...
import { ChevronDown, CircleDollarSign, Receipt, SearchIcon } from 'lucide-react';
import {
  Table,
  TableBody, TableCell,
  TableContainer,
...
const ApartmentsRooms = () => {
  ...
  const [defaultApartmentId, setDefaultApartmentId] = useState<number | null>(null);
  ...
  const [sendingStatusChanges, setSendingStatusChanges] = useState<{ [key: number]: boolean }>({});
  ...
  const handleSave = async () => {
      ...
      try {
          ...
          const createBills = Object.keys(sendingStatusChanges).flatMap((roomId) => {
              const room = rooms[roomId]; // Access room directly by ID
              if (!room) return []; // Skip if room not found

              const rentValue = (rentChanges[roomId] ?? room.rent_amount) / room.tenants.length;
          ...
          if (!roomRes.ok) {
              throw new Error(`Change rooms rent failed with status ${roomRes.status}`);
          }
          ...
          const billRes = await fetch(`${API_BASE_URL}/bills/bulk`, {
              ...
              body: JSON.stringify({ bills: createBills, apartmentId: selectedApartmentId, billPeriod: yearAndMonth }),
              credentials: 'include',
          });
          if (!billRes.ok) {
  ...
  const handleRentChangeSelected = (rentValue: number) => {
    ...
    try {
      const updates = selectedRooms.reduce((acc, roomId) => {
  ...
  const handleCancel = () => {
    const confirmCancel = window.confirm('คุณแน่ใจหรือไม่ว่าต้องการยกเลิกการเปลี่ยนแปลง?');
    if (confirmCancel) {
      setRentChanges([]);
      setSendingStatusChanges([]);
  ...
  const groupByFloor = (rooms) => {
    return rooms.reduce((acc, room) => {
      ...
      const floorData = acc.find(f => f.floor === floor) || { floor, rooms: [] };
  ...
  useEffect(() => {
    ...
    const handleBeforeUnload = (e) => {
      if (Object.keys(rentChanges).length > 0 || Object.keys(sendingStatusChanges).length > 0) {
        e.preventDefault();
  ...
  }, [userId, rentChanges, sendingStatusChanges, getToken]);
  ...
  const fetchData = async () => {
      try {
          ...
          if (apartmentsData.length > 0) {
              setSelectedApartmentId(defaultData.default_apartment_id || apartmentsData[0].apartment_id);
          }
      } catch (error) {
  ...
  const handleSetDefault = async () => {
    ...
    try {
      await axios.post(`${API_BASE_URL}/api/set-default-apartment`, {
  ...
  if (loading) return <App title="บิลค่าเช่า"><div className="flex justify-center">กำลังโหลด...</div></App>;
  if (apartments.length === 0) return <App title="บิลค่าเช่า"><div className="flex justify-center">ไม่เจอที่พัก</div></App>;

  const selectedApartment = apartments.find(a => a.apartment_id === selectedApartmentId);
  const apartmentRooms = Object.values(rooms).filter(room => room.apartment_id === selectedApartmentId);
  const sortedRooms = sortRoomsByNumber(apartmentRooms);
  const totalPages = Math.ceil(sortedRooms.length / PAGE_SIZE);
  const paginatedRooms = sortedRooms.slice((currentPage - 1) * PAGE_SIZE, currentPage * PAGE_SIZE);
  ...
  return (
    <App title="บิลค่าเช่า">
      ...
      {Object.keys(rentChanges).length > 0 || Object.keys(sendingStatusChanges).length > 0 ? (
        <div className="fixed bottom-4 right-4 bg-yellow-100 p-4 rounded-lg shadow-md flex items-center gap-4 z-10">
          <span className="text-yellow-800">คุณมีการเปลี่ยนแปลงที่ยังไม่ได้บันทึก</span>
          <Button
            onClick={handleSave}
              ...
              >
                {apartments.map(apartment => (
                  <option key={apartment.apartment_id} value={apartment.apartment_id}>
                    {apartment.apartment_name} - {selectedApartment.apartment_address}{apartment.apartment_id === defaultApartmentId && ' (ค่าเริ่มต้น)'}
                  </option>
                ))}
              </select>
              <Button
          ...
          <div className='flex flex-col gap-4'>
            ...
            <div className='flex flex-col gap-4'>
              <div className='flex gap-4'>
                ...
                <Button
                  onClick={handleSave}
                  disabled={isSaving || (Object.keys(rentChanges).length === 0 && Object.keys(sendingStatusChanges).length === 0)}
                  className="bg-[#01BCB4] text-white-500 hover:bg-[#01afa8]"
                >
                ...
                {showSummary && (
                  <div className="fixed inset-0 bg-black-500 bg-opacity-50 flex items-center justify-center z-10">
                    <div className="bg-white-500 p-6 rounded-lg w-96">
                      <h2 className="text-lg font-bold mb-4 text-center">การเปลี่ยนแปลงที่ยังไม่ได้บันทึก</h2>
                      <ul>
                        {Object.keys(rentChanges).map((roomId) => {
                          const room = Object.values(rooms).find((r) => r.id === Number(roomId)); // Convert rooms object to an array using Object.values
                          return (
                            <li key={roomId} className="mb-2">
                              ห้อง {room ? room.room_number : roomId}: ค่าเช่า {rentChanges[roomId]} บาท
                            </li>
                          );
                        })}
                        {Object.keys(sendingStatusChanges).map((roomId) => {
                          const room = Object.values(rooms).find((r) => r.id === Number(roomId)); // Fix here
                          return (
                            <li key={roomId} className="mb-2">
                              ห้อง {room ? room.room_number : roomId}: สถานะการส่ง{' '}
                              {sendingStatusChanges[roomId] ? 'ส่งแล้ว' : 'ยังไม่ส่ง'}
                            </li>
                          );
                        })}
                      </ul>
              ...
              <div className="overflow-x-auto">
                <TableContainer component={Paper} sx={{ minWidth: 300 }}>
                  <Table>
                    <TableHead>
                      <TableRow>
                        <TableCell sx={{ width: 125, border: '1px solid rgba(224, 224, 224, 1)' }} style={{ verticalAlign: 'top' }}>
                          <label className="flex flex-col items-center space-x-2 gap-2">
                            <span>เลือกทั้งตาราง</span>
                            <input
                              type="checkbox"
                              className="form-checkbox h-5 w-5"
                              checked={selectedRooms.length === paginatedRooms.length}
                              onChange={(e) => {
                                if (e.target.checked) {
                        ...
                        <TableCell sx={{ border: '1px solid rgba(224, 224, 224, 1)' }} style={{ verticalAlign: 'top' }}>
                        ...
                        <TableCell style={{ verticalAlign: 'top' }} sx={{ border: '1px solid rgba(224, 224, 224, 1)' }}>
                          <div className="flex flex-col items-center">
                          ค่าเข่า/คน
                          </div>
                        </TableCell>
                        <TableCell sx={{ width: 313, border: '1px solid rgba(224, 224, 224, 1)' }} style={{ verticalAlign: 'top' }}>
                        ...
                        <TableCell sx={{ width: 200, border: '1px solid rgba(224, 224, 224, 1)' }} style={{ verticalAlign: 'top' }}>
                          <div className='flex'>
                            <div className="flex flex-col items-center gap-2">
                              <h4>
                                ส่งบิล
                              ...
                              <Button
                                variant="flat"
                                onClick={() => handleSendingStatusSelected(true)}
                        ...
                        <TableCell
                          sx={{ border: '1px solid rgba(224, 224, 224, 1)' }} component="th" scope="row" >
                          <div className="flex flex-col items-center gap-2 ">
                            <h4>
                              ดาวน์โหลดบิล
                            </h4>
                        ...
                        <TableRow key={room.id}
                          ...
                          <TableCell
                            ...
                            <ul className="mt-1 space-y-1">
                              {room.tenants.map((tenant) => (
                                <li key={tenant.tenant_id} className="text-sm">
                                  {(room.rent_amount / room.tenants.length).toFixed(2)}
                                </li>
                              ))}
                            </ul>
                          </TableCell>
                          <TableCell
                            ...
                            <div className="flex items-center">
                              <Input
                                ...
                                type="number"
                                placeholder="ค่าเช่า"
                                value={(rentChanges[room.id] ?? room.rent_amount).toString()} // Convert to string
                          ...
                          <TableCell
                            sx={{ border: '1px solid rgba(224, 224, 224, 1)' }} component="th" scope="row" >
                            <div className="flex justify-center">
                              <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full whitespace-nowrap ${room.rent_is_paid ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'}`}>
                          ...
                          <TableCell
                            ...
                            <div className="flex items-center gap-1">
                              <Button
                                ...
                                onClick={() => handleSendingStatusChange(room.id, true)}
                              >
